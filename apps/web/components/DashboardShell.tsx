...
function SupportScreen({ data, session }: { data: Workspace; session: DemoSessionResponse }) { const [scenario, setScenario] = useState<RagScenario>("answer"); const options: { value: RagScenario; label: string }[] = [{ value: "answer", label: "Streaming grounded answer" }, { value: "account", label: "Separate account evidence" }, { value: "refusal", label: "Insufficient evidence refusal" }, { value: "handoff", label: "Handoff preview + consent" }, ...errorPrompts.map(({ label, scenario }) => ({ value: scenario, label }))]; return <><Head title="Support tickets" description={`Tickets below are database-backed for ${data.dashboard.organization.name}. Support uses one replaceable development transport boundary; no live provider is connected.`} /><div className="simulated-label"><b>Development mock transport.</b> The stream, citations, refusal, account evidence, and handoff states are deterministic UI fixtures pending backend integration.</div><div className="support-workspace"><div className="full-support"><aside className="panel scenario-panel"><h2>Preview state controls</h2>{options.map((option) => <button className={scenario === option.value ? "active" : ""} onClick={() => setScenario(option.value)} key={option.value}>{option.label}</button>)}</aside><div className="panel support-console"><SupportChat key={scenario} embedded identity={session.identity} forcedScenario={scenario} /></div></div><div className="panel table-wrap support-ticket-list"><div className="panel-head"><h2>Recent support tickets</h2><span className="small muted">Read-only API data · separate from handoff preview</span></div><table className="data-table"><thead><tr><th>Ticket</th><th>Subject</th><th>Requester</th><th>Status</th><th>Updated (UTC)</th></tr></thead><tbody>{data.tickets.length ? data.tickets.map((ticket) => <tr key={ticket.id}><td><strong>{ticket.reference}</strong></td><td>{ticket.subject}</td><td>{ticket.requesterName}</td><td><Status value={ticket.status} /></td><td>{dateTime(ticket.updatedAt)}</td></tr>) : <EmptyRow columns={5} label="No support tickets for this workspace." />}</tbody></table></div></div></>; }
function KnowledgeScreen() { return <><Head title="Knowledge" description="Review public sources, versions, ingestion stages, failures, and evidence search through the development transport preview." /><KnowledgeConsole /></>; }
