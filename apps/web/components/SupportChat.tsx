...
import type { ChatReply, ChatScenario, TenantId } from "@/lib/contracts";
import { relayOpsService } from "@/lib/service";

type Props = { embedded?: boolean; tenantId?: TenantId | null; initialOpen?: boolean; forcedScenario?: ChatScenario };
const prompts: { label: string; scenario: ChatScenario; question: string }[] = [
  ...
  { label: "Why can’t Northstar add a seat?", scenario: "seats", question: "Why can’t I add another technician?" },
...
export function SupportChat({ embedded = false, tenantId = null, initialOpen = false, forcedScenario }: Props) {
  ...
  async function submitScenario(scenario: ChatScenario, text: string) {
    ...
    const result = await relayOpsService.askSupport(text, tenantId, scenario);
  ...
  return <div className="chat-window" role="dialog" aria-label="Simulated Relay support" aria-modal={!embedded}>
    <div className="chat-head"><div><b>Relay support <span className="status-dot" style={{ display: "inline-block" }} /></b><p>Simulated answers · local demo data</p></div>{!embedded && <button className="icon-button" onClick={() => setOpen(false)} aria-label="Close support chat">×</button>}</div>
    <div className="chat-messages" ref={scrollRef} aria-live="polite">
      {!sent && <><div className="chat-intro"><span className="logo-mark" aria-hidden="true" /><h3>How can I help?</h3><p>Try a guided scenario to preview citations, account evidence, refusal, and handoff.</p></div><div className="chat-suggestions">{prompts.map((p) => <button className="suggestion" key={p.scenario} onClick={() => void submitScenario(p.scenario, p.question)}>{p.label} <span aria-hidden="true">→</span></button>)}</div></>}
