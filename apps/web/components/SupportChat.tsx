...
export function SupportChat({ embedded = false, initialOpen = false }: Props) {
  ...
  return <div className="chat-window" role="dialog" aria-label="Relay support" aria-modal={!embedded}>
    <div className="chat-head"><div><b>Relay support <span className="status-dot" style={{ display: 'inline-block' }} /></b><p>Local evidence and optional local Qwen · fictional portfolio demo</p></div>{!embedded && <button className="icon-button" onClick={() => setOpen(false)} aria-label="Close support chat">×</button>}</div>
    <div className="chat-messages" aria-live="polite">
      {!sent && <><div className="chat-intro"><span className="logo-mark" aria-hidden="true" /><h3>How can I help?</h3><p>Answers are accepted only after the local server validates grounded citations. Local Qwen may be unavailable.</p></div><div className="chat-suggestions">{prompts.map((prompt) => <button className="suggestion" key={prompt.label} onClick={() => void ask(prompt.question)} disabled={loading}>{prompt.label} <span aria-hidden="true">→</span></button>)}</div></>}
      {sent && <div className="bubble user">{sent}</div>}
      {loading && <div className="stream-status" role="status"><b>{phase === 'retrieving' ? 'Checking active public evidence' : phase === 'generating' ? 'Validating local Qwen output' : 'Request accepted'}</b><span className="loading-dots"><span /><span /><span /></span><button className="btn btn-quiet small" onClick={cancel}>Cancel</button></div>}
