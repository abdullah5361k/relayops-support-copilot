...
import { buildGroqChatPayload, GROQ_MAX_COMPLETION_TOKENS, GROQ_MODEL, GROQ_REASONING_EFFORT, GenerationProviderError, GroqProvider, OllamaQwenProvider, QWEN_MODEL } from '../support/generation.provider';
import { buildGroundedPrompt, HANDOFF_INCIDENT_DOCUMENTATION_QUESTION, SupportAnswerService } from '../support/support-answer.service';
...
type Mode = 'deterministic' | 'integrated' | 'real-model' | 'real-groq' | 'groq-smoke' | 'groq-regression' | 'groq-diagnose' | 'groq-reasoning-budget-diagnose' | 'groq-reasoning-budget-verify' | 'groq-handoff-prompt-verify' | 'groq-confirm-only-canonical';
...
const GROQ_REASONING_BUDGET_DIAGNOSIS_IDS = ['multi-contact'] as const;
const GROQ_REASONING_BUDGET_VERIFY_IDS = ['multi-contact', 'ack-hour'] as const;
const GROQ_HANDOFF_PROMPT_VERIFY_IDS = ['confirm-only', 'public-handoff'] as const;
const GROQ_CONFIRM_ONLY_CANONICAL_IDS = ['confirm-only'] as const;
type ProviderPayloadBoundary = { canonicalPublicQuestion: boolean; requesterHandoffOrAccountTermsAbsent: boolean; evidenceRecordCount: number };
type RegressionCheck = { id: string; expected: Item['expectedOutcome']; actual: string; providerInvoked: boolean; expectedNoCall: boolean; providerError: boolean; providerErrorCode?: string; providerErrorType?: string; requestBounds?: ProviderRequestDiagnostics; tokenUse?: Pick<GenerationMetrics, 'inputTokens' | 'outputTokens' | 'totalTokens' | 'reasoningTokens'>; citationCount: number; handoffPreviewEvidenceCount: number; citedAnswer: boolean; expectedSourcesCited: boolean; expectedHandoffAvailable: boolean; handoffAvailable: boolean };
...
/** Derives booleans from the already-bounded provider prompt, then discards it; no prompt or data text is retained. */
function providerPayloadBoundary(prompt: string): ProviderPayloadBoundary {
  const question = prompt.match(/QUESTION_START\n([\s\S]*?)\nQUESTION_END/)?.[1] ?? '';
  return { canonicalPublicQuestion: question === HANDOFF_INCIDENT_DOCUMENTATION_QUESTION, requesterHandoffOrAccountTermsAbsent: !/\b(handoff|human|person|tenant|session|account|subscription|seat|ticket|job|create|execut)\b/i.test(question), evidenceRecordCount: [...prompt.matchAll(/^ID: /gm)].length };
}
...
class RecordingProvider implements GenerationProvider {
  ...
  lastPromptBoundary: ProviderPayloadBoundary | undefined;
  ...
  async generate(prompt: string, signal?: AbortSignal, context?: GenerationContext) {
    this.invocations++; this.lastPromptBoundary = providerPayloadBoundary(prompt);
    try {
...
function modeFromArg(value: string | undefined): Mode | null {
  return value === 'deterministic' || value === 'integrated' || value === 'real-model' || value === 'real-groq' || value === 'groq-smoke' || value === 'groq-regression' || value === 'groq-diagnose' || value === 'groq-reasoning-budget-diagnose' || value === 'groq-reasoning-budget-verify' || value === 'groq-handoff-prompt-verify' || value === 'groq-confirm-only-canonical' ? value : null;
}
function evaluationPaceMs(mode: Mode): number {
  const value = Number(process.env.RELAYOPS_EVALUATION_PACE_MS ?? 0);
  if ((mode !== 'real-groq' && mode !== 'groq-smoke' && mode !== 'groq-regression' && mode !== 'groq-diagnose' && mode !== 'groq-reasoning-budget-diagnose' && mode !== 'groq-reasoning-budget-verify' && mode !== 'groq-handoff-prompt-verify' && mode !== 'groq-confirm-only-canonical') || !Number.isFinite(value) || value < 0 || value > 60_000) return 0;
...
function modeLabel(mode: Mode): string {
  ...
  if (mode === 'groq-reasoning-budget-diagnose') return 'real-groq-reasoning-budget-diagnosis';
  if (mode === 'groq-reasoning-budget-verify') return 'real-groq-reasoning-budget-verification';
  if (mode === 'groq-handoff-prompt-verify') return 'real-groq-handoff-prompt-verification';
  if (mode === 'groq-confirm-only-canonical') return 'real-groq-confirm-only-canonical';
  return 'real-groq-openai-gpt-oss-20b';
}

async function main(): Promise<void> {
  const mode = modeFromArg(process.argv[2]);
  if (!mode) throw new Error('Usage: evaluation:(deterministic|integrated|real-model|real-groq|groq-smoke|groq-regression|groq-diagnose|groq-reasoning-budget-diagnose|groq-reasoning-budget-verify|groq-handoff-prompt-verify|groq-confirm-only-canonical)');
  ...
  try {
    const useRealEmbedding = mode === 'integrated' || mode === 'real-model' || mode === 'real-groq' || mode === 'groq-smoke' || mode === 'groq-regression' || mode === 'groq-diagnose' || mode === 'groq-reasoning-budget-diagnose' || mode === 'groq-reasoning-budget-verify' || mode === 'groq-handoff-prompt-verify' || mode === 'groq-confirm-only-canonical';
    ...
    const rawProvider: GenerationProvider = mode === 'real-groq' || mode === 'groq-smoke' || mode === 'groq-regression' || mode === 'groq-diagnose' || mode === 'groq-reasoning-budget-diagnose' || mode === 'groq-reasoning-budget-verify' || mode === 'groq-handoff-prompt-verify' || mode === 'groq-confirm-only-canonical' ? new GroqProvider() : mode === 'real-model' ? new OllamaQwenProvider() : new DeterministicEvaluationProvider();
    ...
    const items = mode === 'groq-smoke' ? set.questions.filter((item) => item.category === 'documentation').slice(0, 1) : mode === 'groq-regression' ? set.questions.filter((item) => GROQ_REGRESSION_IDS.includes(item.id as (typeof GROQ_REGRESSION_IDS)[number])) : mode === 'groq-diagnose' ? set.questions.filter((item) => GROQ_DIAGNOSIS_IDS.includes(item.id as (typeof GROQ_DIAGNOSIS_IDS)[number])) : mode === 'groq-reasoning-budget-diagnose' ? set.questions.filter((item) => GROQ_REASONING_BUDGET_DIAGNOSIS_IDS.includes(item.id as (typeof GROQ_REASONING_BUDGET_DIAGNOSIS_IDS)[number])) : mode === 'groq-reasoning-budget-verify' ? GROQ_REASONING_BUDGET_VERIFY_IDS.map((id) => set.questions.find((item) => item.id === id)).filter((item): item is Item => Boolean(item)) : mode === 'groq-handoff-prompt-verify' ? GROQ_HANDOFF_PROMPT_VERIFY_IDS.map((id) => set.questions.find((item) => item.id === id)).filter((item): item is Item => Boolean(item)) : mode === 'groq-confirm-only-canonical' ? GROQ_CONFIRM_ONLY_CANONICAL_IDS.map((id) => set.questions.find((item) => item.id === id)).filter((item): item is Item => Boolean(item)) : set.questions;
    ...
    if (mode === 'groq-reasoning-budget-diagnose' && (items.length !== GROQ_REASONING_BUDGET_DIAGNOSIS_IDS.length || items.some((item, index) => item.id !== GROQ_REASONING_BUDGET_DIAGNOSIS_IDS[index]))) throw new Error('Groq reasoning-budget diagnosis set no longer matches the remaining failure case');
    if (mode === 'groq-reasoning-budget-verify' && (items.length !== GROQ_REASONING_BUDGET_VERIFY_IDS.length || items.some((item, index) => item.id !== GROQ_REASONING_BUDGET_VERIFY_IDS[index]))) throw new Error('Groq reasoning-budget verification set no longer matches the repeated/cited cases');
    if (mode === 'groq-handoff-prompt-verify' && (items.length !== GROQ_HANDOFF_PROMPT_VERIFY_IDS.length || items.some((item, index) => item.id !== GROQ_HANDOFF_PROMPT_VERIFY_IDS[index]))) throw new Error('Groq handoff-prompt verification set no longer matches the answerable/control cases');
    if (mode === 'groq-confirm-only-canonical' && (items.length !== GROQ_CONFIRM_ONLY_CANONICAL_IDS.length || items.some((item, index) => item.id !== GROQ_CONFIRM_ONLY_CANONICAL_IDS[index]))) throw new Error('Groq confirm-only canonical set no longer matches the final authorized case');
    ...
    if (mode === 'groq-diagnose') {
      for (const item of items) {
        ...
        const bounded = (maxEvidence: number) => {
          ...
          return { requestBytes: bytes(JSON.stringify(payload)), boundedPromptBytes, schemaBytes: bytes(JSON.stringify(schema)), evidenceRecordCount: Math.min(maxEvidence, evidence.length), reservedTokenBound: Math.ceil(boundedPromptBytes / 4) + GROQ_MAX_COMPLETION_TOKENS };
        };
    ...
    for (const [index, item] of items.entries()) {
      ...
      const citationSources = new Set([...response.citations.map((entry) => entry.sourceLogicalId), ...response.handoffPreviewEvidence.map((entry) => entry.sourceId)]); const retrievalHit = !item.expectedSources.length || item.expectedSources.every((source) => citationSources.has(source));
      ...
      const coverage = response.state !== 'ANSWERED' || response.accountEvidence.length > 0 || response.citations.length > 0 || response.handoffPreviewEvidence.length > 0;
      ...
      const handoffOffer = response.handoffAvailable && response.handoffPreviewEvidence.length > 0 && response.citations.length === 0 && response.accountEvidence.length === 0;
      const unsupported = response.state === 'ANSWERED' && response.citations.length === 0 && response.accountEvidence.length === 0 && !handoffOffer;
      const stale = response.citations.some((entry) => entry.excerpt.toLowerCase().includes('four business hours'));
      const namespace = response.citations.some((entry) => entry.sourceLogicalId.startsWith('private-')) || response.handoffPreviewEvidence.some((entry) => entry.sourceId.startsWith('private-'));
      ...
      details.push({ id: item.id, category: item.category, expected: item.expectedOutcome, actual: response.state, retrievalHit, citationCount: response.citations.length, handoffPreviewEvidenceCount: response.handoffPreviewEvidence.length, tool: response.accountToolPlan?.tool ?? null, handoffAvailable: response.handoffAvailable, providerInvoked, providerError, providerErrorCode: failure?.providerErrorCode, providerErrorType: failure?.providerErrorType, latencyMs: Math.round(latency) });
      if (mode === 'groq-regression' || mode === 'groq-diagnose' || mode === 'groq-reasoning-budget-diagnose' || mode === 'groq-reasoning-budget-verify' || mode === 'groq-handoff-prompt-verify' || mode === 'groq-confirm-only-canonical') regressionChecks.push({ id: item.id, expected: item.expectedOutcome, actual: response.state, providerInvoked, expectedNoCall: GROQ_REGRESSION_NO_CALL_IDS.has(item.id) || mode === 'groq-confirm-only-canonical', providerError, providerErrorCode: failure?.providerErrorCode, providerErrorType: failure?.providerErrorType, requestBounds, tokenUse: successfulMetrics ? { inputTokens: successfulMetrics.inputTokens, outputTokens: successfulMetrics.outputTokens, totalTokens: successfulMetrics.totalTokens, reasoningTokens: successfulMetrics.reasoningTokens } : undefined, citationCount: response.citations.length, handoffPreviewEvidenceCount: response.handoffPreviewEvidence.length, citedAnswer: item.id === 'ack-hour' && response.state === 'ANSWERED' && response.citations.length > 0 && citationValid, expectedSourcesCited: retrievalHit && citationValid, expectedHandoffAvailable: item.handoffAvailable ?? false, handoffAvailable: response.handoffAvailable });
    ...
    const reasoningBudgetPass = mode === 'groq-reasoning-budget-diagnose' && regressionChecks.length === 1 && regressionChecks[0]!.actual === 'ANSWERED' && regressionChecks[0]!.providerInvoked && !regressionChecks[0]!.providerError && regressionChecks[0]!.citationCount > 0 && provider.invocations === 1 && provider.errors === 0 && provider.provider === 'groq' && provider.model === GROQ_MODEL;
    const reasoningBudgetVerificationPass = mode === 'groq-reasoning-budget-verify' && regressionChecks.length === GROQ_REASONING_BUDGET_VERIFY_IDS.length && regressionChecks.every((check) => check.actual === check.expected && check.providerInvoked && !check.providerError && check.citationCount > 0) && regressionChecks.some((check) => check.id === 'ack-hour' && check.citedAnswer) && provider.invocations === GROQ_REASONING_BUDGET_VERIFY_IDS.length && provider.errors === 0 && provider.provider === 'groq' && provider.model === GROQ_MODEL;
    const handoffPromptVerificationPass = mode === 'groq-handoff-prompt-verify' && regressionChecks.length === GROQ_HANDOFF_PROMPT_VERIFY_IDS.length && regressionChecks.every((check) => check.actual === check.expected && check.providerInvoked && !check.providerError && check.citationCount > 0 && check.handoffAvailable === check.expectedHandoffAvailable) && provider.invocations === GROQ_HANDOFF_PROMPT_VERIFY_IDS.length && provider.errors === 0 && provider.provider === 'groq' && provider.model === GROQ_MODEL;
    const confirmOnlyCanonicalPass = mode === 'groq-confirm-only-canonical' && regressionChecks.length === 1 && regressionChecks[0]!.actual === 'ANSWERED' && !regressionChecks[0]!.providerInvoked && !regressionChecks[0]!.providerError && regressionChecks[0]!.citationCount === 0 && regressionChecks[0]!.handoffPreviewEvidenceCount > 0 && regressionChecks[0]!.expectedSourcesCited && regressionChecks[0]!.expectedHandoffAvailable && regressionChecks[0]!.handoffAvailable && provider.lastPromptBoundary === undefined && provider.invocations === 0 && provider.errors === 0 && provider.provider === 'groq' && provider.model === GROQ_MODEL && aggregate.handoffMutationsBeforeConfirmation === 0;
    const report = mode === 'groq-smoke' ? { kind: 'real-groq-minimal-smoke', provider: 'groq', model: GROQ_MODEL, authenticated: provider.invocations === 1 && provider.errors === 0, validatedCitedAnswer: aggregate.citationValidity === 1 && aggregate.citationCoverage === 1 && aggregate.outcomeRate === 1, citationCount: totals.reduce((sum, value) => sum + value.citationValid, 0), tokenUse: aggregate.tokenUse, rateLimit: aggregate.rateLimit, latencyMs: aggregate.latencyMs, note: 'No question, evidence, answer, account value, or credential is printed.' } : mode === 'groq-regression' ? { kind: 'real-groq-current-code-regression', provider: 'groq', model: GROQ_MODEL, fixedModelIdentity: provider.provider === 'groq' && provider.model === GROQ_MODEL, passed: regressionPass, cases: regressionChecks, zeroProviderErrors: provider.errors === 0, validatedCitedAnswer: regressionChecks.some((check) => check.id === 'ack-hour' && check.citedAnswer), safeOutcomes: { unsupportedClaimRate: aggregate.unsupportedClaimRate, staleVersionViolations: aggregate.staleVersionViolations, namespaceViolations: aggregate.namespaceViolations, tenantViolations: aggregate.tenantViolations, handoffMutationsBeforeConfirmation: aggregate.handoffMutationsBeforeConfirmation }, tokenUse: aggregate.tokenUse, rateLimit: aggregate.rateLimit, latencyMs: aggregate.latencyMs, note: 'No question, evidence text, answer text, account value, session material, or credential is printed.' } : mode === 'groq-diagnose' ? { kind: 'real-groq-sanitized-diagnosis', provider: 'groq', model: GROQ_MODEL, fixedModelIdentity: provider.provider === 'groq' && provider.model === GROQ_MODEL, currentCases: regressionChecks, requestBounds: Object.fromEntries(diagnosisBounds), preReductionComparison: { caseId: 'multi-contact', priorObservedStatusClass: 'bad_request', priorEvidenceRecordCount: 4, currentEvidenceRecordCount: 2 }, note: 'Only sanitized provider code/type and numerical request/schema/token bounds are printed; no response body, question, evidence, answer, account value, session material, or credential is retained.' } : mode === 'groq-reasoning-budget-diagnose' ? { kind: 'real-groq-reasoning-budget-diagnosis', provider: 'groq', model: GROQ_MODEL, fixedModelIdentity: provider.provider === 'groq' && provider.model === GROQ_MODEL, strictSchema: true, generationConfig: { reasoningEffort: GROQ_REASONING_EFFORT, maxCompletionTokens: GROQ_MAX_COMPLETION_TOKENS, schemaRetry: 'disabled' }, passed: reasoningBudgetPass, case: regressionChecks[0], note: 'One provider call only. No question, evidence text, answer text, account value, session material, credential, or provider body is printed.' } : mode === 'groq-reasoning-budget-verify' ? { kind: 'real-groq-reasoning-budget-verification', provider: 'groq', model: GROQ_MODEL, fixedModelIdentity: provider.provider === 'groq' && provider.model === GROQ_MODEL, strictSchema: true, generationConfig: { reasoningEffort: GROQ_REASONING_EFFORT, maxCompletionTokens: GROQ_MAX_COMPLETION_TOKENS, schemaRetry: 'disabled' }, passed: reasoningBudgetVerificationPass, cases: regressionChecks, validatedCitedAnswer: regressionChecks.some((check) => check.id === 'ack-hour' && check.citedAnswer), note: 'Exactly two provider calls: repeated former-error case plus proven cited control. No question, evidence text, answer text, account value, session material, credential, or provider body is printed.' } : mode === 'groq-handoff-prompt-verify' ? { kind: 'real-groq-handoff-prompt-verification', provider: 'groq', model: GROQ_MODEL, fixedModelIdentity: provider.provider === 'groq' && provider.model === GROQ_MODEL, strictSchema: true, generationConfig: { reasoningEffort: GROQ_REASONING_EFFORT, maxCompletionTokens: GROQ_MAX_COMPLETION_TOKENS, schemaRetry: 'disabled' }, passed: handoffPromptVerificationPass, cases: regressionChecks, note: 'Exactly two provider calls: answerable handoff-worded public documentation case plus public-handoff control. Handoff availability remains server-owned; no question, evidence text, answer text, account value, session material, credential, or provider body is printed.' } : mode === 'groq-confirm-only-canonical' ? { kind: 'deterministic-confirm-only-handoff-boundary', configuredProvider: 'groq', model: GROQ_MODEL, fixedModelIdentity: provider.provider === 'groq' && provider.model === GROQ_MODEL, passed: confirmOnlyCanonicalPass, case: regressionChecks[0], providerInvocations: provider.invocations, providerErrors: provider.errors, handoffMutationsBeforeConfirmation: aggregate.handoffMutationsBeforeConfirmation, note: 'No provider call is permitted or made. Only boolean/numeric checks are printed; no question, evidence, answer, account value, session material, credential, or provider body is retained.' } : { evaluationSet: `support-${set.version}`, mode: modeLabel(mode), provider: { provider: provider.provider, model: provider.model }, evaluationPaceMs: paceMs, thresholds: { deterministic: 'retrieval/outcome/tool/handoff >= 0.90; citation validity/coverage = 1.00; all violation and mutation counts = 0', realGroq: 'report-only quality measurement; citation, stale, namespace, tenant, account/tool, and handoff safety violations must always be zero' }, aggregate, byCategory, details };
    ...
    if (mode === 'groq-reasoning-budget-diagnose' && !reasoningBudgetPass) process.exitCode = 1;
    if (mode === 'groq-reasoning-budget-verify' && !reasoningBudgetVerificationPass) process.exitCode = 1;
    if (mode === 'groq-handoff-prompt-verify' && !handoffPromptVerificationPass) process.exitCode = 1;
    if (mode === 'groq-confirm-only-canonical' && !confirmOnlyCanonicalPass) process.exitCode = 1;
