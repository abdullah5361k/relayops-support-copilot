...
type Mode = 'deterministic' | 'integrated' | 'real-model' | 'real-groq' | 'groq-smoke' | 'groq-regression' | 'groq-diagnose' | 'groq-reasoning-budget-diagnose' | 'groq-reasoning-budget-verify' | 'groq-handoff-prompt-verify';
...
function modeFromArg(value: string | undefined): Mode | null {
  return value === 'deterministic' || value === 'integrated' || value === 'real-model' || value === 'real-groq' || value === 'groq-smoke' || value === 'groq-regression' || value === 'groq-diagnose' || value === 'groq-reasoning-budget-diagnose' || value === 'groq-reasoning-budget-verify' || value === 'groq-handoff-prompt-verify' ? value : null;
}
function evaluationPaceMs(mode: Mode): number {
  const value = Number(process.env.RELAYOPS_EVALUATION_PACE_MS ?? 0);
  if ((mode !== 'real-groq' && mode !== 'groq-smoke' && mode !== 'groq-regression' && mode !== 'groq-diagnose' && mode !== 'groq-reasoning-budget-diagnose' && mode !== 'groq-reasoning-budget-verify' && mode !== 'groq-handoff-prompt-verify') || !Number.isFinite(value) || value < 0 || value > 60_000) return 0;
...
async function main(): Promise<void> {
  ...
  if (!mode) throw new Error('Usage: evaluation:(deterministic|integrated|real-model|real-groq|groq-smoke|groq-regression|groq-diagnose|groq-reasoning-budget-diagnose|groq-reasoning-budget-verify|groq-handoff-prompt-verify)');
  ...
  try {
    const useRealEmbedding = mode === 'integrated' || mode === 'real-model' || mode === 'real-groq' || mode === 'groq-smoke' || mode === 'groq-regression' || mode === 'groq-diagnose' || mode === 'groq-reasoning-budget-diagnose' || mode === 'groq-reasoning-budget-verify' || mode === 'groq-handoff-prompt-verify';
    ...
    for (const [index, item] of items.entries()) {
      ...
      if (mode === 'groq-regression' || mode === 'groq-diagnose' || mode === 'groq-reasoning-budget-diagnose' || mode === 'groq-reasoning-budget-verify' || mode === 'groq-handoff-prompt-verify') regressionChecks.push({ id: item.id, expected: item.expectedOutcome, actual: response.state, providerInvoked, expectedNoCall: GROQ_REGRESSION_NO_CALL_IDS.has(item.id), providerError, providerErrorCode: failure?.providerErrorCode, providerErrorType: failure?.providerErrorType, requestBounds, tokenUse: successfulMetrics ? { inputTokens: successfulMetrics.inputTokens, outputTokens: successfulMetrics.outputTokens, totalTokens: successfulMetrics.totalTokens, reasoningTokens: successfulMetrics.reasoningTokens } : undefined, citationCount: response.citations.length, citedAnswer: item.id === 'ack-hour' && response.state === 'ANSWERED' && response.citations.length > 0 && citationValid, expectedHandoffAvailable: item.handoffAvailable ?? false, handoffAvailable: response.handoffAvailable });
