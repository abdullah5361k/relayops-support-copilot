...
import type { GenerationContext, GenerationMetrics, GenerationProvider, ProviderStatus } from '../support/generation.provider';
import { GROQ_MODEL, GroqProvider, OllamaQwenProvider, QWEN_MODEL } from '../support/generation.provider';
...
type Mode = 'deterministic' | 'integrated' | 'real-model' | 'real-groq' | 'groq-smoke';
...
function modeFromArg(value: string | undefined): Mode | null {
  return value === 'deterministic' || value === 'integrated' || value === 'real-model' || value === 'real-groq' || value === 'groq-smoke' ? value : null;
}
function evaluationPaceMs(mode: Mode): number {
  const value = Number(process.env.RELAYOPS_EVALUATION_PACE_MS ?? 0);
  if ((mode !== 'real-groq' && mode !== 'groq-smoke') || !Number.isFinite(value) || value < 0 || value > 60_000) return 0;
...
async function main(): Promise<void> {
  ...
  if (!mode) throw new Error('Usage: evaluation:(deterministic|integrated|real-model|real-groq|groq-smoke)');
  ...
  try {
    const useRealEmbedding = mode === 'integrated' || mode === 'real-model' || mode === 'real-groq' || mode === 'groq-smoke';
    ...
    const rawProvider: GenerationProvider = mode === 'real-groq' || mode === 'groq-smoke' ? new GroqProvider() : mode === 'real-model' ? new OllamaQwenProvider() : new DeterministicEvaluationProvider();
    ...
    const items = mode === 'groq-smoke' ? set.questions.filter((item) => item.category === 'documentation').slice(0, 1) : set.questions;
    const paceMs = evaluationPaceMs(mode);
    const beforeTickets = await prisma.supportTicket.count(); const category = new Map<string, CategoryMetrics>(); const details: unknown[] = [];
