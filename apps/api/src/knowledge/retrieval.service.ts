...
type Candidate = { id: string; logicalId: string; title: string; content: string; heading: string | null; section: string | null; page: number | null; anchor: string | null; rank: number };
...
export class KnowledgeRetrievalService {
  ...
  async searchPublic(query: string, embedder: EmbeddingProvider, limit = 5): Promise<Evidence[]> {
    ...
    const semantic = await this.prisma.$queryRaw<Candidate[]>(Prisma.sql`
      SELECT c.id, s.logical_id AS "logicalId", s.title, c.content, c.heading, c.section, c.page, c.anchor,
