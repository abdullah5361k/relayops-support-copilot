...
The resulting seven-case bounded current-code regression used the same fixed strict-schema configuration and had **zero provider errors**, a validated cited control answer, no stale-beta provider call, and zero unsupported/stale/namespace/tenant/pre-confirmation-mutation violations. It is nevertheless **not a fully passed regression**: `confirm-only` was safely refused after a successful provider call rather than its historical expected `ANSWERED` state. This is an outcome variation, not a provider/schema error; no answer or citation was invented and no retry was sent. Do not claim a fully matched current-code pass until that grounded-outcome difference is resolved safely.
